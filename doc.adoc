...
| `_Arf`	| `flag`	| appropriate Search or Replace Flags ORed togethers
|===
...
| `const _Arf`	| `flag`	| appropriate Search or Replace Flags ORed togethers
|===
...
`_Af` is type and an enumerator type. The type is only used in the declarations of `_afa` and `_afb` as the type of parameter `func`. The emumerator type holds enumerator constants that can be passed in `func`, each representing a boolean function.
...
The `_Af` type should large enough to the range of values of `_Af`'s constants.
...
`_AF_NX` performs NXOR (inverted exclsive disjunction);
