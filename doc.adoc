...
`arb` is a data modification utility that replaces the first, the last or all occurrences of object `b` with object `c` in object `a`.
...
| `As`		| `sc`		| size of the object `c` in bytes
| `_Arf`	| `flag`	| appropriate Search or Replace Flags ORed togethers
|===
...
This type is merely used for the parameter `` 
==== 5.4.2. _Asf -- Array Search Flags
==== 5.4.3. _Arf -- Array Replace Flags
==== 5.4.4. _afa -- Array Function Assignment
`_afa` is an auxillary function that performs a selected operation on one or two objects and assigns the result to a specified pointer.

*Synopsis*:
```c
char* _afa(const char* r, char* const a, char* const b)
```
